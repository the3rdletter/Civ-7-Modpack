...
import SaveLoadData, { DEFAULT_SAVE_GAME_INFO, QueryCompleteEventName } from '/core/ui/save-load/model-save-load.js';
...
class MainMenu extends Component {
    ...
    onInitialize() {
        if (Network.supportsSSO()) {
			...
			<fxs-vslot class="carousel-outer w-full hidden">
				...
				<div class="carousel-thumb-bg carousel-outer w-full bg-primary-4 hidden">
        ...
        this.buildInfo.classList.value = "main-menu-build-info absolute font-body-sm text-accent-2 hidden"; // Add hidden
        ...
        if (Network.supportsSSO()) {
            ...
            this.profileHeader.classList.add("absolute", "top-20", "right-20", "w-auto", "main-menu__profile-header", "hidden"); // Add hidden
            ...
            this.motdDisplay.classList.value = "motd-box absolute flex bottom-0 l-0 w-full justify-center font-body-sm text-accent-2 text-center hidden"; // Add hidden
    ...
    onAttach() {
        ...
        if (Network.isFullAccountLinked()) {
            buttonList.push({ name: "LOC_UI_MP_SOCIAL_BUTTON_LABEL", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs())
                this.showSocialPage(); } });
        }
        buttonList.push({ name: "LOC_MAIN_MENU_META_PROGRESSION", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs())
            this.showProfilePage(); } });
        buttonList.push({ name: "LOC_UI_CONTENT_MGR_TITLE", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs())
                this.openExtras(); } });
        buttonList.push({ name: "LOC_UI_LINK_ACCOUNT_SUBTITLE", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs())
        ...
        if (Network.supportsSSO()) {
            ...
            this.enableMainMenuButtonbyName("LOC_MAIN_MENU_EVENTS", Online.LiveEvent.isEventsEnabled(), this.getAccountLinkPromptMsg());
    ...
    isSelectedPromoInteractable() {
        ...
        if (this.carouselItems.length > 0) {
            ...
            isInteractable = selectedPromo.isInteractable;
        }
    ...
    onQueryComplete(event) {
        switch (event.detail.result) {
            ...
            case SerializerResult.RESULT_OK:
                ...
                this.currentAutosave = SaveLoadData.saves.reduce((prev, current) => prev.saveTime > current.saveTime ? prev : current, DEFAULT_SAVE_GAME_INFO);
                const continueItem = MustGetElement(".continue-item");
                const isDisabled = this.currentAutosave == DEFAULT_SAVE_GAME_INFO || this.currentAutosave.isMissingMods || this.currentAutosave.isLackingOwnership;
    ...
    onSaveLoadClosed() {
        ...
        SaveLoadData.querySaveGameList(SaveLocations.LOCAL_STORAGE, SaveTypes.SINGLE_PLAYER, SaveLocationCategories.AUTOSAVE, SaveFileTypes.GAME_STATE);
    }
    ...
    goContinue() {
        if (this.currentAutosave != DEFAULT_SAVE_GAME_INFO && !this.currentAutosave.isMissingMods && !this.currentAutosave.isLackingOwnership) {
            SaveLoadData.handleLoadSave(this.currentAutosave, ServerType.SERVER_TYPE_NONE);
        }
    }
    showLegalDocuments() {
    ...
    processSelectedPromo() {
        ...
        if (selectedPromo.autoRedeemOnShow) {
            ...
            this.carouselItems[this.selectedCarouselItem].autoRedeemOnShow = false;
        }
    }
    toggleCarouselMode() {
        ...
        if (this.isCarouselExpanded()) {
            ...
            this.telemetryPromoAction(PromoAction.Interact, this.carouselItems[this.selectedCarouselItem].promoId, "Main Menu Carousel", "Expanded Carousel"); // to expanded
    ...
    onActiveLiveEvent() {
        this.enableMainMenuButtonbyName("LOC_MAIN_MENU_EVENTS", Online.LiveEvent.isLiveEventActive(), this.getAccountLinkPromptMsg());
    }
    refreshPromos() {
        if (!Network.supportsSSO()) {
            return;
    ...
    onEngineInput(inputEvent) {
        ...
        switch (inputEvent.detail.name) {
            ...
            case 'touch-tap':
                ...
                break;
            case 'shell-action-2':
                if (this.canPerformInputs() && Network.supportsSSO()) {
                    this.onClickedAccount();
                    live = false;
                }
                break;
    ...
    skipToMainMenu() {
        ...
        if (Network.isConnectedToSSO() && Network.isConnectedToNetwork() && !Network.isAccountComplete() && Network.areAllLegalDocumentsConfirmed()) {
