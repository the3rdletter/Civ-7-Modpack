...
//Affinity Colored Ideologies
export const Leu_ColoredIdeologies = new class {
    _data = {
        IsColored: false
    };

    constructor() {
        const modSettings = ModSettingsManager.read("Leu_ColoredIdeologies");
        if (modSettings) {
            this._data = modSettings;
        }
    }
    ...
    save() {
        console.warn("[Leu_ColoredIdeologies] saving..", JSON.stringify(this._data));
        ModSettingsManager.save("Leu_ColoredIdeologies", this._data);
    }

    get IsColored() {
        return this._data.IsColored;
    }

    set IsColored(value) {
        this._data.IsColored = value;
        this.save();
    }
}

//Team Display for Ideologies
export const Leu_TeamIdeologies = new class {
    _data = {
        ShowChevron: true
    };

    constructor() {
        const modSettings = ModSettingsManager.read("Leu_TeamIdeologies");
        if (modSettings) {
            this._data = modSettings;
        }
    }
    ...
    save() {
        console.warn("[Leu_TeamIdeologies] saving..", JSON.stringify(this._data));
        ModSettingsManager.save("Leu_TeamIdeologies", this._data);
    }

    get ShowChevron() {
        return this._data.ShowChevron;
    }

    set ShowChevron(value) {
        this._data.ShowChevron = value;
        this.save();
    }
}