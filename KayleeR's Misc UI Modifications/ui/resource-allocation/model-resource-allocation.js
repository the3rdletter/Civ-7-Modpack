...
class ResourceAllocationModel {
    ...
    update() {
        ...
        playerCities.getCityIds().forEach((cityID) => {
            ...
            this._availableCities.forEach(city => {
                city.currentResources = city.currentResources.sort(ResourceAllocationModel.resourceSorter);
            });
            function citySorter(a, b)
            {
                let typeCompare = a.settlementType.localeCompare(b.settlementType);
                if (typeCompare != 0)
                {
                    return typeCompare;
                }

                let resourceCapCompare = b.resourceCap - a.resourceCap;
                if (resourceCapCompare != 0)
                {
                    return resourceCapCompare;
                }

                return a.id - b.id;
            }
            this._availableCities = this._availableCities.sort(citySorter);
        });
    ...
    static resourceSorter(a, b)
    {
        let classSort = a.classType.localeCompare(b.classType);
        if (classSort != 0)
        {
            return classSort;
        }
    ...
    sortResources()
    {
        this._availableResources = this._availableResources.sort(ResourceAllocationModel.resourceSorter);
        this._availableBonusResources = this._availableBonusResources.sort(ResourceAllocationModel.resourceSorter);
        this._availableFactoryResources = this._availableFactoryResources.sort(ResourceAllocationModel.resourceSorter);
    }
