...
export class MainMenu extends Component { // CD CHANGES: Now exported for future compatibility with other mods
    ...
    onInitialize() {
        ...
        if (Network.supportsSSO()) {
            ...
            this.profileHeader.setAttribute("show-download-assets", UI.shouldShowHighEndAssetsDownloadOption() ? "true" : "false");
            this.Root.insertAdjacentElement("afterbegin", this.profileHeader);
        }
    ...
    onAttach() {
        ...
        this.slot = MustGetElement("#MainMenuSlot", document);
        ...
        if (UI.shouldShowAdditionalContent()) {
            buttonList.push({ name: "LOC_MAIN_MENU_EXTRAS", audio: "additional-content", buttonListener: () => { if (this.canPerformInputs()) // CD CHANGES: Change localization text
                    this.openExtras(); } });
        }
        else {
        ...
        if (UI.shouldShowHighEndAssetsDownloadOption()) {
            waitForLayout(() => {
                this.profileHeader?.component.showDownloadAssetsDialogConfirm();
            });
        }
    }
    onDetach() {
    ...
    onQueryComplete(event) {
        switch (event.detail.result) {
            ...
            case SerializerResult.RESULT_OK:
                ...
                const continueItem = MustGetElement(".continue-item", document);
    ...
    // CD CHANGES START
    // New dedicated function for creating main menu buttons:
    ...
    // CD CHANGES START
    showSocialPage() { // Called from cd-better-main-menu.js
        Animations.cancelAllChainedAnimations();
    ...
    build3DScene() {
        ...
        for (let attemptsLeft = 3; attemptsLeft >= 0; attemptsLeft--) {
            ...
            if (leaderIndex == -1) {
                ...
                leaderIndex = UI.randomInt(0, leaderData.length - 1);
            }
            ...
            else if (leaderData[leaderIndex].isLocked || !leaderData[leaderIndex].isOwned) {
                continue;
            }
